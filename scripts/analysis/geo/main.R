...
########################################################################################
population_orig <- get_population(year = 2019) %>%
  dplyr::rename(
    CVE_MUN = CLAVE,
    CVE_ENT = CLAVE_ENT,
    NOM_MUN = MUN
...
population_summary <- population %>%
  ...
  tidyr::spread(SEXO, POB_2019) %>%
  dplyr::mutate(
    POB_2019 = Hombres + Mujeres
  ) %>%
  dplyr::select(
    -Hombres, -Mujeres
  ) %>%
...
cities_summary %>%
  ...
  dplyr::select(
    CVE_ENT, CVE_MUN, NOM_MUN.x, NOM_MUN.y
  )
...
write.csv(cities_summary, config$data$summary$cities_es, row.names = FALSE)
